...
export default function SalesHistory({ saleId }: { saleId: string }) {
  ...
  const generateBill = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:8000/api/v1/sales/${saleId}/generate-bill`, {
        method: "POST",
        headers: {
      ...
      if (data.status === 200) {
        if (window.showToast) {
          window.showToast.success("Bill generated and sent successfully")
        } else {
      ...
      if (window.showToast) {
        window.showToast.error("Failed to generate bill")
      } else {
  ...
  if (error || !sale) {
    return (
      <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded">{error || "Sale not found"}</div>
    )
  }
    ...
    <>
      ...
      <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white px-4 pb-3 pt-4 dark:border-gray-800 dark:bg-white/[0.03] sm:px-6">
        <div className="flex flex-col gap-2 mb-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h3 className="text-lg font-semibold text-gray-800 dark:text-white/90">Sale Details</h3>
            <p className="text-sm text-gray-500 dark:text-gray-400">Transaction ID: {sale._id}</p>
            ...
            <table className="table-auto w-full">
              ...
              <thead className="border-gray-100 dark:border-gray-800 border-y">
                <tr>
                  ...
                  <th className="py-3 font-medium text-gray-500 text-start text-xs dark:text-gray-400 px-4">Amount</th>
                </tr>
              </thead>
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                {sale.products.map((product) => (
                  <tr key={product._id}>
                    <td className="py-3 px-4 text-gray-700 text-sm dark:text-gray-300">{product.brand || "N/A"}</td>
                    <td className="py-3 px-4 text-gray-700 text-sm dark:text-gray-300">{product.size || "N/A"}</td>
                    <td className="py-3 px-4 text-gray-700 text-sm dark:text-gray-300">{product.type || "N/A"}</td>
                    <td className="py-3 px-4 text-gray-700 text-sm dark:text-gray-300">{product.quantity}</td>
        ...
        <div className="mt-6 border-t border-gray-100 dark:border-gray-800 pt-4">
          <div className="flex flex-col gap-2 sm:items-end">
            <div className="flex justify-between sm:w-64">
              ...
              <span className="font-medium text-gray-800 dark:text-white">₹{sale.final_discount.toFixed(2)}</span>
            </div>
            <div className="flex justify-between sm:w-64">
            ...
            <div className="flex justify-between sm:w-64">
              ...
              <span className="font-medium text-gray-800 dark:text-white">{sale.payment_method}</span>
            </div>
            <div className="flex justify-between sm:w-64">
              <span className="text-gray-600 dark:text-gray-400">Bill Status:</span>
              <span className={`font-medium ${sale.bill_generated ? "text-green-600" : "text-yellow-600"}`}>
